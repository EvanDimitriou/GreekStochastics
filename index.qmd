...
**Background:** The unconfoundedness of the Randomised Control Trials leads to high internal validity, but due to their small sample sizes they are not representative of the target population. The large sample sizes of the observational studies lead to representativeness, but they lack internal validity due to confounding. Gaussian Processes for multi-task learning was mainly developed and used in the context of geostatistics. The Linear Model of Coregionalization can be viewed as a generative approach for constructing valid covariance functions formulated as a sum of separable kernels. Under this modeling approach the outcomes are expressed as linear combinations of independent random functions, allowing them to be correlated and share information.

**Objectives:** We use multi-output Gaussian Processes to estimate heterogeneous treatment effects when combining experimental and observational studies.
...
where $\textbf{m}\in \mathbb{R}^D$ is a vector the components of which are the mean functions $\{m_d(\textbf{x})\}_{d=1}^D$ of each components and $\textbf{K}$ is a positive matrix valued function with dimension $ND \times ND$ and with entries $\left(\textbf{K}(\textbf{x}_i, \textbf{x}j) \right){d,d'}$ dor $i,j=1,\cdots,N$ and for $d,d'=1,\cdots,D$. More explicitly,
...
$$\left(\textbf{K}(\textbf{x},\textbf{x}')\right)_{d,d'} = k(\textbf{x},\textbf{x}')k_T(d,d')$$ where $k$,$k_T$ are scalar kernels on $\mathcal{X}\times \mathcal{X}$ and $\{1,\cdots,D\} \times \{1,\cdots,D\}$.

Equivalently, one can consider the matrix expression

$$
\textbf{K}(\textbf{x},\textbf{x}')=k(\textbf{x},\textbf{x}')\textbf{B}
$$
...
We want to combine these two data sets to obtain an unbiased estimate of the CATE on the observational study.
...
In the ICM, the outputs are expressed as linear combinations of independent random functions and in our particular case as the weighted sum of two latent functions, described with the same kernel.
...
The covariance for $\textbf{f}(\textbf{x})$ is given in terms of the covariance functions for $u_q^i(\textbf{x})$, $i=1,2$ and $q=1,2,3,4$
...
\end{bmatrix}\otimes k(\textbf{x},\textbf{x}') = \textbf{B}\otimes k(\textbf{x},\textbf{x}')
$$

The rank of the matrix $\textbf{B}$ is equal to the number of independent latent functions we use to describe the outcomes, i.e. equal to 2.

Following this parameterisation, we can obtain the CATE of each dataset by simply taking their difference of the potential outcomes, i.e.
...
Apart from estimation the Conditional Average Treatment Effect, we also need to to estimate the uncertainty of our prediction.
...
In order to estimate the CATE we trained a rank 2 ICM where each potential outcome is considered an output for the model. We also estimated the the CATE using a technique proposed by Kallus et al (2018) where the RCT is used to deconfound the CAATE estimated on the observational study. This "deconfoundig"is done using a bias/confounding function, which is a function trying to capture the bias introduced on the observational data by the hidden confounders. This function has the form:
...
-   Multi-output GPs are proven to be effective in estimating heterogeneous treatment effects when combining observational and experimental studies.
...
The credible intervals obtained by the ICM are very narrow, proving that the ICM is over-confident when predicting the CATE.

![](images/CI2.png){fig-align="center"}

![](images/CIzoom.png){fig-align="center"}

Moreover, the model treats the different sources of data as equal, i.e. datapoints from the RCT are considered exactly equivalent as datapoint coming from the observational study. However, it would be worth it to investigate the result of the estimation if we would assign different weight to individuals coming from different studies.

![](images/weight.png){fig-align="center" width="406"}

Last, the ICM trained to estimate the CATE in the simulation study was defined using the RBF kernel for the independent latent functions. However, we should investigate if the choice of another kernel would yield better results.
